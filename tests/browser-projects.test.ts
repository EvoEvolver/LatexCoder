...
Details at \url{https://example.com}. \tldr{The method combines two stages.}`;
    ...
    await macroEditor.waitFor();
    assert.equal(await macroEditor.textContent(), "\\section{Overview}");
    await page.keyboard.insertText("Background");
    assert.equal(await macroEditor.textContent(), "\\section{Background}");
    await overviewNode.getByRole("button", { name: "Done", exact: true }).click();
    await page.waitForFunction(() => [...document.querySelectorAll(".tree-writer-label")].some(element => element.textContent === "Background"));

    overviewNode = page.locator(".tree-writer-node").filter({ has: page.locator(".tree-writer-node-button", { hasText: "Background" }) }).first();
    await overviewNode.locator(":scope > .tree-writer-node-row").getByLabel("Add node", { exact: true }).click();
    await overviewNode.getByRole("button", { name: "Add subsection", exact: true }).click();
    const insertedEditor = overviewNode.locator(".tree-writer-editor .cm-content");
    await insertedEditor.waitFor();
    assert.match(await insertedEditor.textContent(), /\\subsection\{\}/);
    await page.keyboard.insertText("Motivation");
    await overviewNode.getByRole("button", { name: "Done", exact: true }).click();
    await page.waitForFunction(() => [...document.querySelectorAll(".tree-writer-label")].some(element => element.textContent === "Motivation"));
    const editedMain = await (await page.request.get(`${base}/v1/files?project=${id}&path=main.tex`)).text();
    assert.match(editedMain, /\\section\{Background\}/);
    assert.match(editedMain, /\\subsection\{Motivation\}/);
